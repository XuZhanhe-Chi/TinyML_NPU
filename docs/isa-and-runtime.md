...
| 2 | `0x08` | hw_status | 最后一次 `venus_run_bundle` 返回的 STATUS |
| 3 | `0x0C` | sample_count | 已完成样本数，v0.1.0 为 120 |
| 4 | `0x10` | label_correct | top1 与样本 label 相同的次数 |
| 5 | `0x14` | ref_top1_match | top1 与参考 logits top1 相同的次数 |
| 6 | `0x18` | max_abs_error | 所有样本 signed INT8 最大绝对误差 |
| 7 | `0x1C` | total_mismatches | 所有样本逐 byte mismatch 总数，仅诊断 |
| 8 | `0x20` | first_failure_sample | 首个 runtime/ref/tolerance 失败样本，`UINT32_MAX` 表示无 |
| 9 | `0x24` | first_failure_top1 | 首个失败样本的硬件 top1 |
| 10 | `0x28` | first_failure_expected_top1 | 首个失败样本的参考 top1 |
| 11 | `0x2C` | first_failure_label | 首个失败样本的真实 label |
| 12 | `0x30` | status | 最终寄存器 STATUS snapshot |
| 13 | `0x34` | debug0 | 最后一个样本的 NPU active cycles |
| 14 | `0x38` | debug1 | 最终状态机 snapshot |
| 15 | `0x3C` | total_cycles | 所有样本 NPU active cycles 之和 |

PASS 条件是 `code == 0`、`sample_count >= 100`、v0.1.0 的 120 个样本全部完成、`ref_top1_match == 120`、`label_correct >= 117` 且 `max_abs_error <= 5`。`total_mismatches` 可以非零，因为它只记录固定点舍入差异。
